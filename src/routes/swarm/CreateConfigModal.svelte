<script lang="ts">
	...
	import { Sliders, TriangleAlert } from 'lucide-svelte';
	...
	async function handleSubmit() {
		errors = {};
		let hasErrors = false;
		if (!name.trim()) { errors.name = 'Config name is required'; hasErrors = true; }
		if (!data) { errors.data = 'Config content is required'; hasErrors = true; }
		if (hasErrors) return;
		...
		try {
			...
			const response = await fetch(appendEnvParam('/api/swarm/configs', envId), {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ name: name.trim(), data })
			});
			const body = await response.json();
			if (!response.ok) throw new Error(body.details || body.error || 'Failed to create config');
			resetForm();
			open = false;
			onSuccess?.();
		} catch (err) {
...
<Dialog.Root bind:open onOpenChange={(isOpen) => { if (isOpen) focusFirstInput(); else handleClose(); }}>
	<Dialog.Content class="max-w-lg">
		<Dialog.Header>
			<Dialog.Title class="flex items-center gap-2"><Sliders class="w-5 h-5" />Create config</Dialog.Title>
			<Dialog.Description>Unlike secrets, config content isn't encrypted at rest and can be read back later.</Dialog.Description>
		</Dialog.Header>
		...
		<div class="space-y-4 mt-4">
			...
			<div class="space-y-2">
				<Label for="config-data">Content *</Label>
				<Textarea
					id="config-data"
					bind:value={data}
					rows={10}
		...
		<Dialog.Footer class="mt-6">
			...
			<Button onclick={handleSubmit} disabled={creating}>{#if creating}Creating...{:else}Create config{/if}</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
